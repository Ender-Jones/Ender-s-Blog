...
3. console 行为契约 `docs/CONSOLE.md` 末节「拟定决议」最终点头(关日即翻页 + 04:00 前不得再关 + 不做 unclose + 录入台永不禁用; project 词保留)— 点头后 Claude 才动代码.
4. 想一篇「搜索磁铁」post(中或英): ghostty-cheatsheet 那种一搜即中、停留时间长的工具文. ⚠ 站还 noindex, 收益等换正式域名后兑现, 先定选题. 候选(2026-07-12 头脑风暴, 全部只写工具/方法, 不碰未发表数字):
   - MediaPipe Face Mesh **478 landmark 索引速查图**(常年被搜 "face mesh landmark index", 现有资料散在 GitHub issues; 正好是他 pipeline 的第一步)
   - **交叉验证下比较两个模型**的统计检验 cheatsheet(corrected paired t-test / Nadeau–Bengio + Hedges' g, 带可抄 python; 每个研究生都会搜)
   - zh: **Cloudflare Pages + D1 + Access 全流程踩坑记**(刚亲历, 中文内容近乎空白, Workers/Pages 混淆坑真实存在)
   - zh: **数据泄漏 checklist**(subject-level vs record-level split 对照表 — 和 "reproducible evaluation" 人设同频)
   - **UBFC-Phys 数据集实用手册**(目录结构/T1-T3 语义/采样率/坑; 只写数据集事实)
   - pyVHR **rPPG 方法一表流**(OMIT/POS/CHROM 对照 + 最小代码)
