...
**第二阶段 — console 真联网(D1 + Access + PAT). 照下面四步点; 任何一步和描述对不上, 截图丢给 Claude:**
...
5. 保存后在应用详情页复制 **Application Audience (AUD) Tag**(一长串十六进制, 在编辑页第一屏/Overview)→ 这就是 `ACCESS_AUD` 的值.
6. `ACCESS_TEAM_DOMAIN` 的值 = `<team名>.cloudflareaccess.com`(忘了 team 名: Zero Trust → Settings → Custom Pages 顶部写着).
</details>
...
1. dash.cloudflare.com → **Workers & Pages** → `ender-s-blog` → **Settings** → **Variables and Secrets**(旧名 Environment variables)→ 给 **Production** 加三条:

   | 名 | 值 | 类型 |
   |---|---|---|
   | `ACCESS_TEAM_DOMAIN` | `<team名>.cloudflareaccess.com` | Plaintext |
   | `ACCESS_AUD` | Step 2 复制的 AUD tag | Plaintext |
   | `GITHUB_TOKEN` | Step 3 的 PAT | **Secret** |
...
**机器侧(已就绪, 等你点)**
4. 部署第二阶段: 截图级步骤已写在上面「部署 · 第二阶段」— 照着点, Database ID 丢给 Claude 收尾(回填 wrangler.toml + console 切 `/api`).
