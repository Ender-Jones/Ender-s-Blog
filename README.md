...
3. console 一个字的决定: project 字段改名 **wip**(语义"摸了但没完", 与 done 互斥不重复)— 换/不换, 见 `docs/CONSOLE.md` 末节. (关日即翻页已拍板, 连同"字段语义上录入台"一起, Claude 下个对话实现.)
4. 「搜索磁铁」post 选题(⚠ 站还 noindex, 收益等换域名后兑现; 动笔前先做 SERP 调查确认没被人写死).
