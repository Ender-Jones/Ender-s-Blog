...
⚠ 一定走 **Pages** 流程, 不要走 **Workers** 流程. 分辨: 如果界面标题是 "Configure your Worker project"、让你填 "Deploy command"(`npx wrangler deploy`)—— 那是 Workers, 会报 `Missing entry-point to Worker script`, 退出重来. Pages 流程没有 deploy 命令这一栏, 建完自动部署.

1. [dash.cloudflare.com](https://dash.cloudflare.com) → **Workers & Pages** → **Create** → 顶部切到 **Pages** 标签页 → **Connect to Git** → 选 `Ender-Jones/Ender-s-Blog`(首次要授权 GitHub).
2. 构建设置: Framework preset 选 **Astro**; Build command `npm run build`; Build output directory `dist`. **没有 deploy 命令栏**(有就说明进错流程了). 环境变量加一条 `NODE_VERSION` = `22`.
3. 点 **Save and Deploy**, 两三分钟后得到 `https://ender-s-blog.pages.dev` — 之后每次 git push 自动重新部署.
4. 站带着 `noindex`, 搜索引擎不会收录 — 随便看.

第一阶段里 `/console/` 仍是 LOCAL MODE(记在浏览器里), 三个 `/api/*` 会 500 — 正常, 它们等的是第二阶段(`wrangler.toml` 里 D1 绑定现在是注释掉的, 所以静态部署干净).
