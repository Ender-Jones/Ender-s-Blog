...
5. 补写历史条目时, 把「事后重建 + 证据来源」写进 **commit message**(范例 `c8ae52a`), 不在文件里
   留注释 —— 文件内注释会跟着 md 一路冒到 console 预览卡顶上, 而 git 记录既更难篡改也不碍眼.
   (2026-07-25 改此口径: 06/07 两处旧注释已删, legacy 04/05 的注释按"永不回改"留着.)
